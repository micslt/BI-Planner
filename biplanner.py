...
def calculate_points_and_save_to_csv(csv_file):
    ...
    with open("reifegrad.csv", "r", newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            organisation.append(sum([1 if answer == "ja" else 0 for answer in row[1:7]]) / 6 * 100)
            ressourcen.append(sum([1 if answer == "ja" or answer in ["5"] else 0.75 if answer == "4" else 0.5 if answer == "3" else 0.25 if answer == "2" else 0 for answer in row[7:13]]) / 6 * 100)
            dateninfrastruktur.append(sum([1 if answer == "ja" else 0 for answer in row[13:19]])/6*100)
            analytik.append(sum([1 if answer == "ja" or answer in ["5"] else 0.75 if answer == "4" else 0.5 if answer == "3" else 0.25 if answer == "2" else 0 for answer in row[19:25]])/6*100)
            governance.append(sum([1 if answer == "ja" else 0 for answer in row[25:35]])/10*100)
            total.append(sum([organisation[0], ressourcen[0], dateninfrastruktur[0], analytik[0], governance[0]]) / 5)
